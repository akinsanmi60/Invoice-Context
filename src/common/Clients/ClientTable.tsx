...
function ClientTable({ showAdvanceSearch = false, clientSelected = false }) {
  const { clientData, setOpenClientSelect, setClientSelector } =
    useClientDataContext();
  const { register, watch } = useForm<SearchProp>();
  const [pageNumber, setPageNumber] = useState(0);
  const formData = watch();
  const incomingData = clientData;
  const [currentItems, setCurrentItems] = useState<NewClientProp[]>([]);

  useEffect(() => {
    setCurrentItems(clientData);
  ...
  useEffect(() => {
    ...
    console.log({ filterData }, "Effects");
  ...
  const handleSelect = useCallback(
    (item: { id: string }) => {
      setClientSelector(item.id);

      setTimeout(() => {
        setOpenClientSelect(false);
      }, 50);
    },
    [setClientSelector, setOpenClientSelect],
  );

  console.log({ clientData, currentItems, incomingData });
