...
import InvoiceTopBar from "../../common/Invoice/InvoiceTopBar";
...
function InvoiceDetailScreen() {
  const { stateData, toggleNavbar, setEscapeOverflow } = useAppContext();
  const { showNavbar, initLoading } = stateData;
  const [invoiceForm, setInvoiceForm] = useState<InvoiceProp | null>(null);
  const [isViewMode, setIsViewMode] = useState(false);
  const [isExporting, setIsExporting] = useState(false);
  const { selectClientData, setOpenClientSelect } = useClientDataContext();
  const { setOpenProductSelect } = useProductDataContext();
  const { setSettingModalOpen } = useAppContext();
  ...
  const toggleViewMode = () => {
    if (invoiceForm?.statusIndex !== "1" && isViewMode) {
      toast({
        description: "You can only edit on Draft Mode",
        status: "warning",
        duration: 6000,
        isClosable: true,
  ...
  const openSettingModal = () => {
    if (invoiceForm?.statusIndex !== "1" && isViewMode) {
      toast({
        description: "You can only change Setting on Draft Mode",
        duration: 6000,
        isClosable: true,
        position: "bottom-left",
        status: "warning",
  ...
  const addEmptyProduct = useCallback(() => {
    ...
    setInvoiceForm(prev => {
      ...
      updatedData.totalAmount = sumTotalAmount(
        ...
        sumTotalTaxes(updateTaxes).toString(),
      );
      return updatedData;
    });
  }, []);

  const onDeleteProduct = useCallback((prodID: string) => {
    setInvoiceForm(prev => {
      const updatedData = { ...prev };
  ...
  const handlerInvoiceValue = useCallback(
    (event: string | number | Date | null, keyName: string) => {
      const value = keyName === "due date" ? new Date(event as Date) : event;
  ...
  const handlerProductValue = useCallback(
    (event: { target: { value: any } }, keyName: string, productID: string) => {
      ...
      const updatedData = { ...invoiceForm };
      const updateProducts = [...invoiceForm!.products];
      ...
      setInvoiceForm(updatedData as InvoiceProp);
    },
    [invoiceForm],
  );
  ...
  const handlerInvoiceClientValue = useCallback(
    (event: string | Date | null | number, keyName: string) => {
                ...
                >
                  {!isViewMode ? (
                    <input
                      ...
                      onChange={e =>
                        handlerInvoiceClientValue(e.target.value, "name")
                      }
                    />
                  ) : (
                ...
                >
                  {!isViewMode ? (
                    <input
                      ...
                      className={defaultInputSmStyle}
                      value={selectClientData?.billingAddress}
                      onChange={e =>
                    ...
                    selectClientData?.billingAddress
                  )}
                </div>
                <div
                  className={
                ...
                >
                  {!isViewMode ? (
                    <input
                      ...
                      className={defaultInputSmStyle}
                      value={selectClientData?.mobileNo}
                      onChange={e =>
                        handlerInvoiceClientValue(e.target.value, "mobileNo")
                      }
                    />
                  ) : (
                    selectClientData?.mobileNo
                  )}
                </div>
                <div
                  className={
                ...
                >
                  {!isViewMode ? (
                    <input
                      ...
                      className={defaultInputSmStyle}
                      value={selectClientData?.email}
                      onChange={e =>
                        handlerInvoiceClientValue(e.target.value, "email")
                      }
                    />
                  ) : (
                    selectClientData?.email
                  )}
                </div>
              </div>
            </div>
                ...
                <div className="font-title flex-1 text-right">
                  {!isViewMode ? (
                    <input
                      ...
                      onChange={e =>
                        handlerInvoiceValue(e.target.value, "invoiceNo")
                      }
                    />
                  ) : (
              ...
              <div className="flex flex-row justify-between items-center mb-1">
                ...
                <div className="font-title flex-1 text-right">
                  <DatePicker
                    ...
                    onChange={date => handlerInvoiceValue(date, "createdDate")}
                    disabled={true}
                    className={
                      !isViewMode
              ...
              <div className="flex flex-row justify-between items-center mb-1">
                ...
                <div className="font-title flex-1 text-right">
                  <DatePicker
                    ...
                    onChange={date => handlerInvoiceValue(date, "dueDate")}
                    disabled={isViewMode}
                    className={
                      !isViewMode
                        ? defaultInputSmStyle + " border-gray-300 text-right"
                ...
                <div className="flex flex-row justify-between items-center mb-1">
                  ...
                  <div className="font-title flex-1 text-right">
                    <input
                      ...
                      className={defaultInputSmStyle + " text-right"}
                      onChange={e =>
                        handlerInvoiceValue(e.target.value, "currencyUnit")
                      }
                    />
                  </div>
                </div>
          ...
          <div className="py-2 px-4">
            ...
            {invoiceForm?.products?.map((product, index) => (
              ...
              >
                <div
                  className={
                    isExporting
                      ? "font-title w-1/4 text-right pr-8 flex flex-row "
            ...
            {!isViewMode && (
              <div className="flex flex-col sm:flex-row rounded-lg sm:visible w-full px-4 py-2 items-center sm:justify-end">
                <div className="font-title w-full sm:w-1/4 text-right sm:pr-8 flex flex-row sm:block mb-1">
                  <Button size="sm" block={true} onClick={addEmptyProduct}>
                    <PlusCircleIcon style={IconStyle} className="h-5 w-5" />
                    Add Empty Product
                  </Button>
                </div>
                <div className="font-title w-full sm:w-1/4 text-right sm:pr-8 flex flex-row sm:block mb-1">
                  <Button size="sm" block={true} onClick={openChooseProduct}>
            ...
            >
              <div
                className={
                  isExporting
                    ? "font-title w-1/4 text-right pr-9 flex flex-row justify-end text-sm "
            ...
            {!isViewMode && (
              <div className="flex flex-col sm:flex-row rounded-lg sm:visible w-full px-4 py-2 items-center sm:justify-end">
                <div className="font-title w-full sm:w-1/4 text-right sm:pr-8 flex flex-row sm:block mb-1">
                  <Button size="sm" block={true} onClick={addPercentageTax}>
                    <TaxesIcon style={IconStyle} className="h-5 w-5" />
                    Add Taxes (%)
                  </Button>
                </div>
                <div className="font-title w-full sm:w-1/4 text-right sm:pr-8 flex flex-row sm:block mb-1">
                  <Button size="sm" block={true} onClick={addEmptyTax}>
                    <DollarIcon
                      style={IconStyle}
